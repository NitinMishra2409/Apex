...
Apexlog is a React 18 / Vite SPA backed by Supabase Auth, Postgres, and row-level security. Vercel serves the static build and four server routes. NVIDIA Riva provides transcription; Groq supplies Coach chat and speech. [README](../README.md) owns setup, and this document owns implementation structure.
...
Preserve existing user data when fixing these issues.
